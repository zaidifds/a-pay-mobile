...
type DepositByBankScreenNavigationProp =
...
export default function DepositByBankScreen() {
  const { theme } = useTheme();
  const { t, isRTL } = useTranslation();
  const navigation = useNavigation<DepositByBankScreenNavigationProp>();

  const [expandedSection, setExpandedSection] = useState<string | null>(
    'international',
  );
  const animationValues = useRef({
    international: new Animated.Value(1),
    domestic: new Animated.Value(0),
  }).current;
